...
为了确保广告 SDK 能正常下载和编译，必须在**宿主 Android 项目**中进行以下配置。
...
广告 SDK 内部定义了 Label 和备份策略，容易与主工程冲突，**必须**添加 `tools:replace`。
...
**Q: 广告显示空白?**
> **A:** 
> 1. 检查 Logcat 日志，过滤 `AdSystem`。
> 2. 检查 Flutter Widget 的 `height` 是否足够（原生广告至少 250+）。
> 3. 检查广告位 ID 类型是否匹配（不要把 Banner ID 传给 Native）。
